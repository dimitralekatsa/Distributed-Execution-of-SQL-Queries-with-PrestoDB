...
# Directories
INPUT_DIR = os.path.expanduser("~/tpcds-kit/tools/data/")  # Folder where .dat files are stored
OUTPUT_DIR = os.path.expanduser("~/tpcds-kit/tools/tpcds_csv/")  # Folder where .csv files will be stored

# Ensure the output directory exists
os.makedirs(OUTPUT_DIR, exist_ok=True)

def convert_dat_to_csv():
    """ Convert all .dat files in INPUT_DIR to .csv and store them in OUTPUT_DIR """
    for file_name in os.listdir(INPUT_DIR):
        if file_name.endswith(".dat"):
            table_name = file_name.replace(".dat", "")

            if table_name not in TABLE_SCHEMAS:
                print(f"Skipping {file_name} (No schema found)")
                continue

            input_file = os.path.join(INPUT_DIR, file_name)
            output_file = os.path.join(OUTPUT_DIR, f"{table_name}.csv")

            with open(input_file, "r", encoding="utf-8", errors="replace") as dat_file, open(output_file, "w", newline="") as csv_file:
...
if __name__ == "__main__":
    convert_dat_to_csv()
    print("\nAll .dat files have been converted to CSV successfully\n")
