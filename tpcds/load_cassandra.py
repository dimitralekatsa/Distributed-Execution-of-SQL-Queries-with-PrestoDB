...
# Dictionary of correct column order for each table (Manually defined)
TABLE_SCHEMAS = {
    ...
    "customer_demographics": ["cd_demo_sk", "cd_gender", "cd_marital_status", "cd_education_status", "cd_purchase_estimate",
    ...
    "date_dim": ["d_date_sk", "d_date_id", "d_date", "d_month_seq", "d_week_seq", "d_quarter_seq", "d_year", "d_dow",
                 ...
                 "d_holiday", "d_weekend", "d_following_holiday", "d_first_dom", "d_last_dom", "d_same_day_ly",
                 "d_same_day_lq", "d_current_day", "d_current_week", "d_current_month", "d_current_quarter", "d_current_year"],
    "warehouse": ["w_warehouse_sk", "w_warehouse_id", "w_warehouse_name", "w_warehouse_sq_ft", "w_street_number", "w_street_name",
    ...
    "time_dim": ["t_time_sk", "t_time_id", "t_time", "t_hour", "t_minute", "t_second", "t_am_pm", "t_shift", "t_sub_shift",
                 "t_meal_time"],
    "reason": ["r_reason_sk", "r_reason_id", "r_reason_desc"],
    "income_band": ["ib_income_band_sk", "ib_lower_bound", "ib_upper_bound"],
    "item": ["i_item_sk", "i_item_id", "i_rec_start_date", "i_rec_end_date", "i_item_desc", "i_current_price", "i_wholesale_cost",
             "i_brand_id", "i_brand", "i_class_id", "i_class", "i_category_id", "i_category", "i_manufact_id", "i_manufact",
    ...
    "call_center": ["cc_call_center_sk", "cc_call_center_id", "cc_rec_start_date", "cc_rec_end_date", "cc_closed_date_sk",
                    ...
                    "cc_company_name", "cc_street_number", "cc_street_name", "cc_street_type", "cc_suite_number", "cc_city",
    ...
    "web_page": ["wp_web_page_sk", "wp_web_page_id", "wp_rec_start_date", "wp_rec_end_date", "wp_creation_date_sk", "wp_access_date_sk",
                 ...
                 "wp_max_ad_count"],
    "promotion": ["p_promo_sk", "p_promo_id", "p_start_date_sk", "p_end_date_sk", "p_item_sk", "p_cost", "p_response_target",
                  "p_promo_name", "p_channel_dmail", "p_channel_email", "p_channel_catalog", "p_channel_tv", "p_channel_radio",
    ...
    "catalog_returns": ["cr_returned_date_sk", "cr_returned_time_sk", "cr_item_sk", "cr_refunded_customer_sk", "cr_refunded_cdemo_sk",
                        ...
                        "cr_warehouse_sk", "cr_reason_sk", "cr_order_number", "cr_return_quantity", "cr_return_amount", "cr_return_tax",
                        "cr_return_amt_inc_tax", "cr_fee", "cr_return_ship_cost", "cr_refunded_cash", "cr_reversed_charge",
    ...
    "web_returns": ["wr_returned_date_sk", "wr_returned_time_sk", "wr_item_sk", "wr_refunded_customer_sk", "wr_refunded_cdemo_sk",
                    ...
                    "wr_return_quantity", "wr_return_amt", "wr_return_tax", "wr_return_amt_inc_tax", "wr_fee", "wr_return_ship_cost",
                    "wr_refunded_cash", "wr_reversed_charge", "wr_account_credit", "wr_net_loss"],
    "web_sales": ["ws_sold_date_sk", "ws_sold_time_sk", "ws_ship_date_sk", "ws_item_sk", "ws_bill_customer_sk", "ws_bill_cdemo_sk",
    ...
    "store_sales": ["ss_sold_date_sk", "ss_sold_time_sk", "ss_item_sk", "ss_customer_sk", "ss_cdemo_sk", "ss_hdemo_sk", "ss_addr_sk",
...
def clean_csv(file_path, chunksize=100000):
    """ 
    Read CSV in chunks to avoid memory overload. 
    - Converts integer-like floats (e.g., 1234.0 -> 1234).
    - Replaces NaN values with 'NULL'.
    """

    output_file = file_path.replace(".csv", "_cleaned.csv")  # Temporary cleaned file

    def convert_column(chunk):
        """Convert columns:
        - If it looks like an integer (ends in .0), convert to int.
        - If it has missing values, replace with NULL.
        """
        for col in chunk.columns:
            if chunk[col].dtype == "object":  # Process only string/object columns
                if chunk[col].str.match(r"^\d+\.0$").all():  # Check if all values are like 1234.0
                    chunk[col] = chunk[col].str.replace(".0", "", regex=False)  # Remove .0
                    chunk[col] = chunk[col].fillna(0).astype(int)  # Convert safely to int
                else:
                    chunk[col] = chunk[col].replace("nan", "NULL")  # Replace NaN with NULL
        return chunk

    with pd.read_csv(file_path, dtype=str, chunksize=chunksize) as reader:
        for i, chunk in enumerate(reader):
            print(f"Processing chunk {i+1}...")
            chunk = convert_column(chunk)
...
def import_csv_to_cassandra():
    ...
    for file in os.listdir(CSV_DIR):
        if file.endswith(".csv"):
            ...
            # Clean CSV file before importing
            clean_csv(local_path)

            if table_name not in TABLE_SCHEMAS:
                print(f"Skipping {file} (No schema defined)")
                continue
            ...
            # Use correct column order in COPY command
            correct_column_order = TABLE_SCHEMAS[table_name]
            copy_command = f"COPY {CASSANDRA_KEYSPACE}.{table_name}({','.join(correct_column_order)}) FROM '{remote_path}' WITH HEADER = TRUE AND NULL = 'NULL' AND INGESTRATE = 4000;"
